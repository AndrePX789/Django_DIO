...
def lista_eventos(request):
    ...
    evento = Evento.objects.filter(data_evento__gt=data_atual)
...
def submit_evento(request):
    if request.POST:
        ...
        if id_evento:
            ...
            #         FAZER ALTERAÇÃO SE FOR O DONO DO COISA
            # evento = Evento.object.filter(id=id_evento)
...
def delete_evento(request, id_evento):
    #     DELETAR SOMENTE O DONO DO REGISTRO
    # usuario = request.user
...
def json_lista_evento(request):
    evento = Evento.objects.all().values('id', 'titulo', 'local')
